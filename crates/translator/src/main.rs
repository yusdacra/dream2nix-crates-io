...
type SuccessIndex = BTreeMap<String, BTreeSet<String>>;

fn main() {
    let start = Instant::now();

    let raw_index = fs::read_to_string("./gen/index.json").expect("could not read index");
        ...
        let handle = std::thread::spawn(move || {
            for (name, versions) in infos {
                for (version, checksum) in versions {
                    match translate_and_write((&name, &version, &checksum)) {
                        Ok(wrote_to) => {
    ...
    let mut succeeded_crates = SuccessIndex::default();
    ...
    fs::write("./gen/locks/index.json", index).expect("could not write index");
    println!("wrote succeeded crates to './gen/locks/index.json'");

    println!("done in {:.1} seconds", start.elapsed().as_secs_f32());
}

fn translate_and_write((name, version, checksum): (&str, &str, &str)) -> Result<PathBuf> {
    let slib_flake_src = option_env!("SLIB_FLAKE_SRC").unwrap_or("./.");
    let lib =
        format!("(builtins.getFlake (toString {slib_flake_src})).lib.${{builtins.currentSystem}}");
    let func_args =
        format!("{{name = \"{name}\"; version = \"{version}\"; hash = \"{checksum}\";}}");
    let expr = format!("{lib}.dreamLockFor {func_args}");
    // generate dream lock
    let raw_dream_lock = nix(["eval", "--impure", "--json", "--expr", &expr])?;
    let parsed_dream_lock: serde_json::Value = serde_json::from_str(&raw_dream_lock)?;
    let pretty_dream_lock = serde_json::to_string_pretty(&parsed_dream_lock)?;

    // create lock path, create lock dir
    let lock_path: PathBuf = format!("./gen/locks/{name}/{version}/dream-lock.json").into();
